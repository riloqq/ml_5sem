...
class DecisionTree(BaseEstimator, ClassifierMixin):
    ...
    def _fit_node(self, sub_X, sub_y, node, depth=0):
        ...
        if self._max_depth is not None and depth >= self._max_depth: # dop остановки
            node["type"] = "terminal"
            node["class"] = Counter(sub_y).most_common(1)[0][0]
            return

        if np.all(sub_y == sub_y[0]): # fixed: == вместо !=
            node["type"] = "terminal"
            node["class"] = sub_y[0]
            return

        feature_best, threshold_best, gini_best, split = None, None, None, None
        best_categories_map = None  # added для сохранения map
        for feature in range(sub_X.shape[1]): # fixed: с 0 вместо 1
            ...
            if feature_type == "real":
                ...
                for key, current_count in counts.items():
                    ...
                    ratio[key] = current_click / current_count if current_count > 0 else 0 # fixed: click / count
                sorted_categories = [key for key, _ in sorted(ratio.items(), key=lambda x: x[1])] # fixed: x[0] вместо x[1]
            ...
            if left_size < self._min_samples_leaf or (len(sub_y) - left_size) < self._min_samples_leaf: # added проверка
                continue
            ...
            if gini_best is None or gini > gini_best:
                ...
                elif feature_type == "categorical": # fixed "Categorical"
                    threshold_best =  threshold
                    best_categories_map = categories_map.copy()  # fixed: копия map
                else:
                    raise ValueError

        if feature_best is None:
            node["type"] = "terminal"
            node["class"] = Counter(sub_y).most_common(1)[0][0] # fixed: [0][0]
            return
        ...
        self._fit_node(sub_X[~split], sub_y[~split], node["right_child"], depth + 1) # fixed: ~split для right
    ...
    def fit(self, X, y):
        ...
        return self  # added для chainability